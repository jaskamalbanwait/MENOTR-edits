...
sim_data = model_simulator(param_vals_fit(1,1:end),indvar,flag);
st_dev = sqrt(sum(sum((data_input-model_simulator(param_vals_fit,indvar,flag)).^2))/numel(sim_data)); 
...
parfor k = 0:MC_sim_num-1
    k+1
[~,~,~,MC_params(:,k+1),~,~] = lsqnl_fitting_routine(indvar,MC_sims(:,n*k+1:n*k+n),parlim,weights(n*k+1:n*k+n),algsw,flag) %#ok<PFBNS>
end

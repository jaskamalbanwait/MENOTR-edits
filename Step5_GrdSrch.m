...
for i = 1:length(fxparams) 
    ...
    for j = 1:2 %Division of Calculations to the right and left hand sides of the best fit value
        ...
        for k = 1:length(cparam)
           ...
           [~,~,~,grd_params((((i-1)*length(cparam)*2)+((j-1)*length(cparam))+k),:),grd_chisq((((i-1)*length(cparam)*2)+((j-1)*length(cparam))+k),1),~]=lsqnl_fitting_routine(indvar,data_input,parlim,weights,algsw,flag); 
